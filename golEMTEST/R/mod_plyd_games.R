...
#'
#' @noRd 
mod_plyd_games_server <- function(input, output, session){
  ns <- session$ns
  output$plyd_table <- renderTable({
    df <- readr::read_csv(file.path(data_dir, input$plyd_user, "games.csv"),
                   col_types = cols(
...
# mod_plyd_games_ui("plyd_games_ui_1")
