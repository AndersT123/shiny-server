...
#'
#' @noRd 
mod_games_table_server <- function(input, output, session, user_name){
  ns <- session$ns
...
# mod_games-table_ui("games-table_ui_1")
    
## To be copied in the server
# callModule(mod_games-table_server, "games-table_ui_1")
...
get_data <- function(user_name) {
  ...
  data <- readr::read_csv(paste0(data_dir,"/", user_name, "/games.csv"),
                   col_types = cols(
